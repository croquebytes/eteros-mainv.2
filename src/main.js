import './style.css';
console.log('MAIN JS RELOADED ' + Date.now());
...
import { eBuy } from './os/apps/eBuy.js';
import { overclockApp } from './os/apps/overclockApp.js';
import { gambitEditor } from './os/apps/gambitApp.js';
import { systemUpdater } from './os/apps/systemUpdater.js';
// NEW APPS (Phase 2)
...
import { overclockSystem } from './state/overclockSystem.js';
...
// windowManager.init(windowLayerEl); // Moved to end
...
windowManager.registerApp(eBuy);

console.log('DEBUG: overclockApp import value:', overclockApp);

try {
  if (overclockApp) {
    windowManager.registerApp(overclockApp);
    console.log('DEBUG: overclockApp registered successfully');
  } else {
    console.error('DEBUG: overclockApp is undefined/null');
  }

} catch (error) {
  console.error('DEBUG: Error registering overclockApp:', error);
}

windowManager.registerApp(gambitEditor);

try {
  windowManager.registerApp(systemUpdater);
} catch (error) {
  console.error('Error registering systemUpdater:', error);
}
...
// Initialize window manager after apps are registered
...
overclockSystem.init(gameState.overclockState);
