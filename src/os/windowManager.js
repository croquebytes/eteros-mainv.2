...
export const windowManager = {
  ...
  openWindow(appId, focus = true) {
    console.log(`openWindow called for ${appId}`, this);
    const entry = this.windows[appId];
    ...
    if (!entry.el) {
      ...
      // Snap Layouts Menu Trigger
      let hoverTimeout;
      maximizeBtn.addEventListener('mouseenter', () => {
        console.log('Maximize button mouseenter');
        hoverTimeout = setTimeout(() => {
          console.log('Triggering showSnapLayoutMenu');
          showSnapLayoutMenu(maximizeBtn, (zone) => {
            this.applySnapLayout(appId, zone);
          });
      ...
      maximizeBtn.addEventListener('mouseleave', () => {
        ...
        hideSnapLayoutMenu(300); // Small delay to allow moving to menu
      });
    ...
    // Add open animation
    entry.el.classList.remove('animating-close');
    entry.el.classList.add('animating-open');
    entry.el.addEventListener('animationend', () => {
      entry.el.classList.remove('animating-open');
    }, { once: true });
  ...
  minimizeWindow(appId) {
    ...
    // Animate closing
    entry.el.classList.add('animating-close');
  ...
  closeWindow(appId) {
    ...
    if (entry && entry.el) {
      ...
      entry.el.addEventListener('animationend', () => {
        entry.el.style.display = 'none';
        entry.el.classList.remove('os-window--active', 'os-window--minimized', 'os-window--maximized', 'animating-close');
  ...
  /**
   * Apply a specific snap layout zone to a window
   */
  applySnapLayout(appId, zone) {
    const entry = this.windows[appId];
    if (!entry || !entry.el) return;
    ...
    const pixelZone = {
      x: zone.x * viewportWidth,
      y: zone.y * workHeight,
      width: zone.w * viewportWidth,
      height: zone.h * workHeight,
      id: zone.id
    };

    // Store pre-snap state if not already snapped
    const state = getDesktopState();
    const windowState = state.windows[appId];
    if (windowState && !windowState.snap && !windowState.isMaximized) {
      this.beforeSnapState[appId] = {
    ...
    // Apply snap
    const updates = applySnapToWindow(entry.el, pixelZone);
    updateWindowState(appId, updates);
