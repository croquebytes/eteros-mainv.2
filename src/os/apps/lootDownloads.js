...
window.unequipFromHero = (heroId, slot, rootEl) => {
  ...
  if (result.success) {
    ...
    // Find the window content if rootEl is missing (which it is when called from onclick)
    const targetEl = rootEl || document.querySelector('[data-app-id="lootDownloads"] .window-content');
    if (targetEl) {
      render(targetEl.parentNode); // render expects the window body (parent of window-content) or just the container
    }
  }
};
