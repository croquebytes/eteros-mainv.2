...
const APPS = [
  ...
  { id: 'researchLab', label: 'Research Lab', icon: '🔬' },
  ...
  { id: 'soulSummoner', label: 'Summon Heroes', icon: '🔮' },
  { id: 'eBuy', label: 'E-Buy', icon: '🛍️' },
  { id: 'defragger', label: 'Defragger', icon: '🧹' },
  { id: 'firewallDefense', label: 'Firewall Defense', icon: '🛡️' },
  { id: 'cosmeticTerminal', label: 'Aesthetic Terminal', icon: '🎨' },
  { id: 'systemMonitor', label: 'System Monitor', icon: '📊' },
  { id: 'resourceTracker', label: 'Resource Tracker', icon: '💾' },
  { id: 'petTerminal', label: 'Daemonling Pet', icon: '👾' },
  { id: 'fakeBrowser', label: 'NetSim Browser', icon: '🌐' },
...
export function createDesktop() {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    wallpaper.style.transform = `translate(${-x}px, ${-y}px) scale(1.05)`; // Scale to prevent edges showing
  });
  ...
  APPS.forEach((app) => {
    ...
    glyph.textContent = app.icon || app.label[0] || '?';
    ...
    if (isMobile) {
      ...
      // Select sound on single click
      icon.addEventListener('click', () => {
        if (!icon.classList.contains('desktop-icon--dragging')) {
          // Optional: selection sound?
        }
      });
    }
  ...
  setTimeout(() => {
    notificationSystem.show({
      title: 'System Ready',
      message: 'ReincarnOS v2.0 initialized. Welcome back, User.',
      icon: '🚀',
      type: 'success'
    });
  }, 1000);
...
function createMobileNav() {
  ...
  const items = [
    ...
    },
    {
      ...
      icon: '⚔️',
      label: 'Quest',
      action: () => windowManager.openWindow('questExplorer')
    },
    {
      id: 'shop',
      icon: '🛒',
      label: 'Shop',
      action: () => windowManager.openWindow('soulwareStore')
    },
    {
      id: 'apps',
      icon: '📱',
      label: 'Apps',
      action: () => {
        // Toggle start menu or open a specific app drawer
