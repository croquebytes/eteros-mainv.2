...
import { gambitSystem } from './gambitSystem.js';
...
export function startDungeon() {
  ...
  if (gameState.activeDungeonRun) {
    gameState.activeDungeonRun.currentWave = 1;
    gameState.activeDungeonRun.startTime = Date.now();
  }
  ...
  // Start the combat loop with variable speed
  runCombatLoop();
  notify();
}

export function startRaid(target) {
  if (dungeonInterval) return;
  gameState.dungeonState.running = true;
  gameState.dungeonState.isRaid = true;
  gameState.dungeonState.raidTarget = target;
  gameState.dungeonState.timeInWave = 0;
  gameState.wave = 1;
...
function runCombatLoop() {
  ...
  const interval = baseInterval / multipliers.speed;

  tickCombat();

  // Schedule next tick
  dungeonInterval = setTimeout(runCombatLoop, interval);
}

export function stopDungeon() {
  if (dungeonInterval) {
    clearTimeout(dungeonInterval);
...
function spawnEnemies() {
  if (gameState.dungeonState.isRaid) {
    const target = gameState.dungeonState.raidTarget;
    currentEnemies = target.party.map((member, index) => ({
      id: `rival_hero_${index}`,
      name: `${target.name}'s ${member.class}`,
      maxHp: member.level * 50,
      currentHp: member.level * 50,
      atk: member.level * 8,
      def: member.level * 3,
      isBoss: false
    }));
    combatLog.push({ type: 'info', message: `Connected to ${target.name}. Initiating hack...` });
    return;
  }
...
function tickCombat() {
  ...
  for (const hero of activeHeroes) {
    if (now - hero.lastAttackTime >= 2000) { // Attack every 2 seconds
      const battleState = { enemies: aliveEnemies, heroes: activeHeroes };
      const gambitResult = gambitSystem.evaluate(hero, battleState);

      if (gambitResult) {
        executeGambitAction(hero, gambitResult);
        hero.lastAttackTime = now;
      } else {
        // Default behavior (Random Target)
...
function executeGambitAction(hero, result) {
  const { action, target } = result;

  if (action === 'attack') {
    heroAttack(hero, target);
  } else if (action === 'item_potion') {
    // Basic potion logic: Heal 50% HP
...
function heroAttack(hero, enemy) {
  ...
  // Apply Overclock Damage Multiplier
  damage = Math.floor(damage * multipliers.damage);
...
function completeWave() {
  ...
  const xpReward = Math.floor(baseXp * (isBossWave ? 2 : 1) * multipliers.xp);
...
function completeRaid() {
  ...
  // Grant Rewards
  gameState.gold += target.rewards.gold;
  gameState.lifetimeGold += target.rewards.gold;

  const xpPerHero = Math.floor(target.rewards.xp / gameState.heroes.length);
  gameState.heroes.forEach(hero => {
    if (!hero.onDispatch) addXpToHero(hero, xpPerHero);
  });

  if (target.rewards.codeFragments > 0) {
    gameState.resources.codeFragments = (gameState.resources.codeFragments || 0) + target.rewards.codeFragments;
  }

  if (battleNotify) {
    battleNotify(`Raid Successful! +${target.rewards.gold} Gold, +${target.rewards.xp} XP`, 'success');
  }

  gameState.dungeonState.running = false;
  gameState.dungeonState.isRaid = false;
  gameState.dungeonState.raidTarget = null;
  notify();
}
